 ...
 */
export type HorizontalPlacement = 'left' | 'leftAlign' | 'center' | 'right' | 'rightAlign'
...
export function isVerticalPlacementValid({
  ...
  switch (verticalPlacement) {
    ...
    case 'bottom': {
      return triggerBCR.bottom + contentContainerBCR.height + offsetVertical <= window.innerHeight
    }
  }
}
...
export function isHorizontalPlacementValid({
  ...
  switch (horizontalPlacement) {
    ...
    case 'right': {
      return triggerBCR.right + contentContainerBCR.width + offsetHorizontal <= window.innerWidth
    }
...
export function getPlacement({
  ...
  if (validPlacements.find((placement) => JSON.stringify(placement) === JSON.stringify(defaultPlacement))) {
    return defaultPlacement
  }
...
export function getTopPosition({
  ...
  switch (verticalPlacement) {
    ...
    case 'center': {
      return triggerBCR.top + triggerBCR.height / 2 - contentContainerBCR.height / 2
    }
...
export function getLeftPosition({
  ...
  switch (horizontalPlacement) {
    ...
    case 'center': {
      return triggerBCR.left + triggerBCR.width / 2 - contentContainerBCR.width / 2
    }
 ...
 */
export function getTransformOriginClassName(placement?: [VerticalPlacement, HorizontalPlacement]): string {
  if (!placement) {
    return ''
  }
  ...
  if (verticalPlacement === 'top') {
    if (horizontalPlacement === 'leftAlign' || horizontalPlacement === 'right') {
      return 'origin-bottom-left'
    }

    if (horizontalPlacement === 'rightAlign' || horizontalPlacement === 'left') {
