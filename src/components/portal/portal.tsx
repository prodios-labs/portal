...
export function Portal({
  ...
  const portalContainer = useMemoOne(() => {
    const container = typeof window !== 'undefined' ? document.createElement('div') : undefined
  ...
  const { contentVisibility, contentStyle, containerPlacement, setContentContainerPosition } = usePortalPosition({
    visible,
    trigger: triggerRef,
    contentContainer,
    allowedPlacements,
  ...
  return (
    <>
      {contentVisibility === ContentVisibility.INVISIBLE ? <div style={contentStyle}>{content}</div> : null}
      {createPortal(
        <>
          <CSSTransition
          ...
          >
            <div className={clsx(getTransformOriginClassName(containerPlacement))} style={contentStyle}>
              {content}
            </div>
          </CSSTransition>
        </>,
